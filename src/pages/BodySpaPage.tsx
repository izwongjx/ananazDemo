...
export default function BodySpaPage() {
  return (
    <div className="bg-[#FAF9F6] min-h-screen">
      ...
      <section className="bg-dark relative overflow-hidden pt-36 pb-24">
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-10 relative z-10">
          ...
          <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
            ...
            <span className="w-8 h-px bg-gold inline-block" /> Holistic Restoration · 02
          </motion.p>
          <motion.h1 initial={{ opacity: 0, y: 32 }} animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.75, delay: 0.1, ease: [0.22, 1, 0.36, 1] }}
            className="font-display text-5xl lg:text-7xl font-semibold text-white leading-[1.08] max-w-3xl mb-6">
            Body<br /><span className="text-gold italic">Spa.</span>
          </motion.h1>
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }}
            className="font-body text-white/55 text-base leading-relaxed max-w-lg">
            From aromatherapy to ancient Malay wellness rituals — restore your body, calm your mind, and revive your spirit.
          </motion.p>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 lg:px-10 py-20">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <div>
            <p className="font-body text-[10px] tracking-[0.3em] uppercase text-gold mb-2 font-bold">Our Spa Packages</p>
            <h2 className="font-display text-3xl lg:text-4xl font-semibold text-dark">2 Signature Experiences</h2>
          </div>
          <p className="font-body text-xs text-muted hidden lg:block max-w-xs text-right leading-relaxed">
            Select any package to explore the deep restorative process and therapeutic benefits.
          </p>
        </motion.div>

        <div className="divide-y divide-divider">
          {bodySpaTreatments.map((t, i) => (
            <motion.div
              key={t.id}
      ...
      <section className="bg-dark py-24 text-center">
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
          ...
          <Wind className="text-gold mx-auto mb-6" size={24} />
          <h2 className="font-display text-3xl lg:text-4xl font-semibold text-white mb-4">
            Your body deserves restoration, not just rest.
          </h2>
          <p className="font-body text-white/50 text-sm mb-10 leading-relaxed">
            Certified therapists · Pure ingredients · 20 years of experience.
          </p>
          <Link to="/booking"
